...
- The production MVP source is now bound to one persistent GPT Sites project through `.openai/hosting.json`. GPT Sites is an additional frontend hosting target only; GitHub remains the canonical source repository and Supabase remains the single backend for Auth, PostgreSQL, Storage, RLS, and Edge Functions. A build adapter copies the unchanged Vite output into the Sites runtime contract, and Sites receives only browser-safe Vite configuration rather than service-role, payment, or AI provider secrets.
