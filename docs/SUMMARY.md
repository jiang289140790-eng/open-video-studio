...
- The production MVP source is now bound to one persistent GPT Sites project through `.openai/hosting.json`. GPT Sites is an additional frontend hosting target only; GitHub remains the canonical source repository and Supabase remains the single backend for Auth, PostgreSQL, Storage, RLS, and Edge Functions. Sites receives only browser-safe Vite configuration and never receives service-role or third-party provider secrets.
