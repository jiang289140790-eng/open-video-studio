...
- `apps/web/app.js` now includes an MVP i18n layer with registered locales, core term coverage, text dictionaries, attribute dictionaries, and source-text preservation for Chinese, English, Japanese, and Korean. This improves the product surface but is not yet a full translation management system, CMS localization workflow, or per-locale content publishing pipeline.
