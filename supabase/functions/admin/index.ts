...
function normalizeWorkflowCenterConfig(config: unknown) {
  ...
  const input = typeof config === "object" && config ? config as Record<string, unknown> : {};
  const workflows = withDefaultZealmanWorkflows(Array.isArray(input.workflows) ? input.workflows : fallback.workflows);
  return {
...
function withDefaultZealmanWorkflows(workflows: Array<Record<string, unknown>>) {
  const existing = new Set(workflows.map((workflow) => String(workflow.workflowId || "")));
  const zealmanWorkflows = [
    ...
    { workflowId: "workflow-zealman-image-e01-v1", name: "Zealman E01 蒙版图片编辑工作流", type: "comfyui", provider: "zealman_workflow", jsonConfig: { action: "process-generation-job", mediaType: "image", workflowEnv: "ZEALMAN_IMAGE_EDIT_WORKFLOW" }, requiredModels: ["qwen-image-edit"], requiredInputs: ["prompt", "source_image", "mask_image"], outputType: "image", creditPrice: 10, version: "v1", status: "testing", description: "通过 Qwen Image Edit 编辑源图，并按独立蒙版合成以保留未编辑区域。" },
    { workflowId: "workflow-zealman-image-m01-v1", name: "Zealman M01 多图组合工作流", type: "comfyui", provider: "zealman_workflow", jsonConfig: { action: "process-generation-job", mediaType: "image", workflowEnv: "ZEALMAN_IMAGE_COMPOSITION_WORKFLOW" }, requiredModels: ["flux2-klein-9b", "qwen3.6-gguf"], requiredInputs: ["prompt", "source_images"], outputType: "image", creditPrice: 14, version: "v1", status: "testing", description: "通过 D18 工作流组合一至两张参考图，服务端关闭其余演示图片输入。" },
    { workflowId: "workflow-zealman-image-p01-v1", name: "Zealman P01 姿势生成工作流", type: "comfyui", provider: "zealman_workflow", jsonConfig: { action: "process-generation-job", mediaType: "image", workflowEnv: "ZEALMAN_POSE_WORKFLOW" }, requiredModels: ["qwen-image-edit-2509", "qwen-image-controlnet-union", "openpose"], requiredInputs: ["prompt", "identity_image", "pose_image"], outputType: "image", creditPrice: 14, version: "v1", status: "testing", description: "图一提供身份，图二经 OpenPose 提取骨架，再由 Qwen ControlNet 约束单人物姿势。" },
    { workflowId: "workflow-zealman-image-o01-v1", name: "Zealman O01 AI 换装工作流", type: "comfyui", provider: "zealman_workflow", jsonConfig: { action: "process-generation-job", mediaType: "image", workflowEnv: "ZEALMAN_OUTFIT_WORKFLOW" }, requiredModels: ["flux2-klein-9b", "qwen3.6-gguf"], requiredInputs: ["prompt", "person_image", "garment_image"], outputType: "image", creditPrice: 14, version: "v1", status: "testing", description: "复用 D18 多参考编辑能力：图一提供人物，图二只提供服装，服务端强制双图输入并限制只替换服装。" },
    { workflowId: "workflow-zealman-image-f01-v1", name: "Zealman F01 人脸替换工作流", type: "comfyui", provider: "zealman_workflow", jsonConfig: { action: "process-generation-job", mediaType: "image", workflowEnv: "ZEALMAN_FACE_SWAP_WORKFLOW" }, requiredModels: ["flux2-klein-9b", "qwen3.6-gguf"], requiredInputs: ["prompt", "target_image", "identity_image", "identity_consent"], outputType: "image", creditPrice: 14, version: "v1", status: "testing", description: "图一提供目标场景，图二提供经授权身份；服务端要求双图和身份使用同意。" },
...
function defaultToolCatalogConfig() {
  return {
    tools: [
      ...
      { slug: "liblib-image", name: "Liblib 图片生成", category: "image", status: "draft", provider: "liblib_generation", model: "liblib-text2img-v1", workflowId: "workflow-liblib-image-v1", creditCost: 8, route: "./zh/app/generate/", featured: false, versions: [{ version: "v1", changelog: "Liblib text-to-image API workflow", modelVersion: "liblib-text2img-v1", workflowVersion: "workflow-liblib-image-v1", promptVersion: "prompt-image-launch-v1", status: "testing" }] },
...
function defaultWorkflowCenterConfig() {
  return {
    workflows: [
      ...
      { workflowId: "workflow-liblib-image-v1", name: "Liblib 图片生成工作流", type: "api_chain", provider: "liblib_generation", jsonConfig: { action: "process-generation-job", mediaType: "image" }, requiredModels: ["liblib-text2img-v1"], requiredInputs: ["prompt"], outputType: "image", creditPrice: 8, version: "v1", status: "testing", description: "通过 Liblib 开放平台文生图 API 生成图片，启用前需要配置 AccessKey、SecretKey 和模板 UUID。" },
