...
- The seven requested generation capabilities now have one machine-readable coverage matrix and permanent workflow IDs. A01 is fully qualified; E01, G01, M01, O01 and F01 have direct runtime evidence. P01 now has dedicated OpenPose and Qwen ControlNet execution but remains identity-gated. See [AI-WORKFLOW-002](0550-ai-engine/002-workflow-capability-matrix.md).
- E01 manual-mask runtime is qualified. The repository-owned Qwen Image Edit 2509 graph produced a real output on AutoDL, and expanded Gaussian mask blending removed the first run's hard rectangular seam. It remains unpublished until automatic masking and the authenticated credits/refund/Storage/history loop pass.
- G01 has passed direct AutoDL runtime qualification with a 25-frame MP4. M01 also produced a valid 6.15 MB image through fixed D18 inputs. Dedicated P01 executes OpenPose preprocessing and Qwen ControlNet, but controlled review showed the pose-reference identity winning over the source identity; a FaceID/IP-Adapter class identity lock is still required. AutoDL was verified shutdown after qualification.
- O01 has passed direct runtime and visual qualification with the D18 two-reference preset: one person, pose and background were preserved while the garment changed, producing a valid 7.14 MB PNG. It remains unpublished until controlled identity comparison and the complete Supabase qualification gate pass.
- F01 has passed direct runtime and scene-preservation review with a valid 7.26 MB PNG, fixed target/identity inputs and a server-side consent requirement. It remains unpublished until quantitative identity similarity, misuse safeguards and the complete Supabase qualification gate pass; a dedicated identity model remains the preferred production path.
