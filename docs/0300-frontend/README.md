...
- `apps/web/app.js` now includes an MVP i18n layer with registered locales, product-term coverage, text dictionaries, attribute dictionaries, and source-text preservation for Chinese, English, Japanese, and Korean. `npm run verify:i18n` is the quality gate for the current static product surface. This improves the product surface but is not yet a full translation management system, CMS localization workflow, or per-locale content publishing pipeline.
