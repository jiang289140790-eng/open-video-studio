...
- 2026-07-12: MVP product loop trust completion. Separated signed-out functional previews from real account data across Assets, History, My Works, Characters, and Dashboard; required real login for generation/tool-demo/share actions; synced authenticated characters from Supabase; hid Fake Worker from public model selection; repaired Image-to-Video UTF-8 Chinese; marked unfinished operations modules as Beta/development; added 375px/390px/412px/768px mobile guards. Verification: `npm run build` passed; `npm run test` passed with 75 tests; `npm run verify:supabase`, `npm run verify:auth-basic`, and `npm run verify:user-loop` passed; `npm run verify:ai` failed only on Qwen Vision 401 unauthenticated; `npm run verify:mvp` reported ready for small user testing with Telegram and Qwen Vision as non-blocking launch blockers.
